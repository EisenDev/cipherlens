/**
 * Footer — Global CipherLens site footer.
 * Light mode, warm palette, multi-column layout.
 */
export function Footer() {
  const currentYear = new Date().getFullYear();

  const columns = [
    {
      title: 'Product',
      links: ['Features', 'Pricing', 'Changelog', 'Roadmap'],
    },
    {
      title: 'Services',
      links: ['Web Security', 'Repo Security', 'AI Reports', 'Developer Profiles'],
    },
    {
      title: 'Resources',
      links: ['Documentation', 'Source Registry', 'API Reference', 'Blog'],
    },
    {
      title: 'Company',
      links: ['About Us', 'Careers', 'Contact', 'Legal'],
    },
  ];

  return (
    <footer className="bg-bg-dark border-t border-border-warm">
      <div className="container-main py-16">
        {/* Top: Logo + columns */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-10 mb-12">
          {/* Brand */}
          <div className="col-span-2 md:col-span-3 lg:col-span-1">
            <div className="flex items-center gap-2.5 mb-4">
              <div
                className="w-7 h-7 rounded-md flex items-center justify-center text-white text-xs font-bold"
                style={{ background: 'var(--color-accent)' }}
              >
                C
              </div>
              <span
                className="text-base font-semibold tracking-tight"
                style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text-on-dark)' }}
              >
                CIPHERLENS
              </span>
            </div>
            <p className="text-sm leading-relaxed mb-5" style={{ color: 'var(--color-text-on-dark-muted)', fontFamily: 'var(--font-body)' }}>
              AI Powered Security Intelligence Platform.<br />
              Protect What Matters.
            </p>
            {/* Social Links */}
            <div className="flex items-center gap-3">
              {['github', 'twitter', 'linkedin', 'youtube'].map((s) => (
                <a
                  key={s}
                  href={`#${s}`}
                  className="w-8 h-8 rounded-md flex items-center justify-center transition-colors"
                  style={{
                    color: 'var(--color-text-on-dark-muted)',
                    backgroundColor: 'rgba(255,255,255,0.06)',
                  }}
                  aria-label={s}
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
                  </svg>
                </a>
              ))}
            </div>
          </div>

          {/* Link columns */}
          {columns.map((col) => (
            <div key={col.title}>
              <h4
                className="text-xs font-semibold mb-4 tracking-widest uppercase"
                style={{ color: 'var(--color-text-on-dark-muted)', fontFamily: 'var(--font-body)' }}
              >
                {col.title}
              </h4>
              <ul className="flex flex-col gap-2.5">
                {col.links.map((link) => (
                  <li key={link}>
                    <a
                      href={`#${link.toLowerCase().replace(/\s+/g, '-')}`}
                      className="text-sm transition-colors"
                      style={{ color: 'rgba(245,243,238,0.55)', fontFamily: 'var(--font-body)' }}
                      onMouseEnter={(e) => (e.currentTarget.style.color = 'var(--color-text-on-dark)')}
                      onMouseLeave={(e) => (e.currentTarget.style.color = 'rgba(245,243,238,0.55)')}
                    >
                      {link}
                    </a>
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </div>

        {/* Bottom: copyright + legal */}
        <div
          className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-8 border-t"
          style={{ borderTopColor: 'rgba(255,255,255,0.08)' }}
        >
          <p className="text-xs" style={{ color: 'var(--color-text-on-dark-muted)', fontFamily: 'var(--font-body)' }}>
            © {currentYear} CipherLens. All rights reserved.
          </p>
          <div className="flex items-center gap-5">
            {['Privacy Policy', 'Terms of Service', 'Security'].map((item) => (
              <a
                key={item}
                href="#"
                className="text-xs transition-colors"
                style={{ color: 'var(--color-text-on-dark-muted)', fontFamily: 'var(--font-body)' }}
              >
                {item}
              </a>
            ))}
          </div>
        </div>
      </div>
    </footer>
  );
}
